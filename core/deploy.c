...
// Executar deploy
int deploy_execute(const char *project_name, const char *environment, const char *commit_id, deploy_config_t *config) {
    char deploy_dir[4096];
    char commit_path[4096];
    char cmd[4096];

    // Criar diretório de deploy se não existir
    snprintf(deploy_dir, sizeof(deploy_dir), ".clurg/projects/%s/deploy/%s",
    ...
    snprintf(commit_path, sizeof(commit_path), ".clurg/projects/%s/commits/%s.tar.gz",
...
int deploy_log(const char *project_name, const char *environment, const char *commit_id,
    ...
    snprintf(log_dir, sizeof(log_dir), ".clurg/projects/%s/deploy/%s/logs",
             project_name, environment);
