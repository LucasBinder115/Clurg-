...
int clurg_clone(const char *project_name, const char *remote_url) {
    ...
    if (snprintf(cmd, sizeof(cmd), "curl -s \"%s/api/last-commit/%s\"", remote_url, project_name) >= (int)sizeof(cmd)) {
    ...
    if (snprintf(cmd, sizeof(cmd), "curl -s -o \"%s\" \"%s/download/%s/%s\"",
                 archive_path, remote_url, project_name, last_commit_id) >= (int)sizeof(cmd)) {
        fprintf(stderr, "comando muito longo\n");
        return 1;
    }
    ...
    /* Baixar o meta */
    if (snprintf(cmd, sizeof(cmd), "curl -s -o \"%s\" \"%s/download/%s/%s.meta\"",
                 meta_path, remote_url, project_name, last_commit_id) >= (int)sizeof(cmd)) {
        fprintf(stderr, "comando muito longo\n");
        return 1;
    }
    ...
    if (expected_checksum[0] != '\0') {
        ...
        if (snprintf(checksum_cmd, sizeof(checksum_cmd), "sha256sum \"%s\" 2>/dev/null", archive_path) >= (int)sizeof(checksum_cmd)) {
    ...
    if (snprintf(cmd, sizeof(cmd), "tar -xzf \"%s\" -C \"%s\"", archive_path, cwd) >= (int)sizeof(cmd)) {
        fprintf(stderr, "comando muito longo\n");
        return 1;
    }
